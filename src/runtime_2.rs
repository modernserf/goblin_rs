...
mod test {
    ...
    impl Value {
        ...
        fn send(self, selector: &str, stack: &mut Stack, call_stack: &mut CallStack) {
            match self {
                Value::Integer(_) => match selector {
                    ...
                    _ => panic!("does not understand: {}", selector),
                },
    ...
    impl Class {
        ...
        fn add(&mut self, selector: &str, params: Vec<Param>, body: Vec<IR>) {
        ...
        fn get(&self, selector: &str) -> &Handler {
            self.handlers.get(selector).unwrap()
        }
        fn get_ivals(&self) -> usize {
            self.ivals
        }
        fn set_ivals(&mut self, count: usize) {
            self.ivals = count
        }
        fn rc(self) -> Rc<Class> {
            Rc::new(self)
        ...
        fn run(&mut self) -> Value {
            loop {
                match self.call_stack.next() {
                    ...
                    NextResult::Done => return self.stack.pop(),
                }
            }
        }
        fn eval(&mut self, ir: IR) {
            match ir {
    ...
    fn run(code: Vec<IR>, expected: Value) {
        assert_eq!(Interpreter::program(code), expected);
    }
